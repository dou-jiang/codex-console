...
async def refresh_accounts_overview(request: OverviewRefreshRequest):
    ...
    # 使用线程池 + asyncio.gather，在不长时间占用数据库连接的情况下刷新总览
    max_workers = min(ACCOUNT_OVERVIEW_REFRESH_MAX_WORKERS, max(1, len(ids)))
    loop = asyncio.get_running_loop()
    
    with ThreadPoolExecutor(max_workers=max_workers, thread_name_prefix="overview_refresh_sync") as executor:
        tasks = [
